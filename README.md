...
* Options to customize dice definitions, so your D10 can be numbered 0-9 instead of 1-10, or edit any unusued definition into a custom die with a unique value range, dice are "auto-named" by their max value so you can have for example multiple D10 definitions (0 to 9 and 1 to 10) but not fake a D20 that only rolls 20
